#include "SQDI_Sys_T23M_Abstract.hh"
#ifdef INTERFACE
/**
* Created:  2014-11-12
...
// const N WRITE_CHUNK_SIZE = 128;    // In kilobytes
//↓ - good trade off for SSD's/HDD's
const N WRITE_CHUNK_SIZE = 64; // In kilobytes
...
/*
                        ...
                        #### ##    ## ####    ##
*/
template <typename T, N Load_Profiling_Enum> // template<ENUM> Load_Profiling_Enum>
class SQDI_Sys_T23M_Abstract {
   public:
    SQDI_Sys_T23M_Abstract(L write_mode, N accepted_missing_files_count)
        : write_mode_(write_mode)
        , accepted_missing_files_count_(accepted_missing_files_count) {
        if (write_mode) {
            const N prel_buf_size = WRITE_CHUNK_SIZE * 1024;
            ...
            N prel_buf_size = 500000;
            raw_buffer.reserve(prel_buf_size);
            raw_buffer.reset_read_cursor();
        }
    }

    ~SQDI_Sys_T23M_Abstract() {
        flush_writes();
        close_output_file();
    };

    inline void set_immediate_mode(L that = true) { immediate_mode_ = that; }
    ...
    SQDI_Sys_T23M_Abstract& operator|( FlagsEnum what ) {
        switch ( what ) {
            case HISTORY_MODE:
...
########
    */
    ...
    MutatingBuffer<B> raw_buffer;

    inline L get_more_data() {
    ...
    inline arbitrary_return_code load_next_with_retries() {
        ...
        N retries = accepted_missing_files_count_ + 1;
   ...
   private:
    ...
    virtual auto read_page_header(B*) -> B* = 0;
    virtual auto write_page_header(B*) -> B* = 0;
...
// else status == 0 som det ska vara...
    ...
    void read_page_header_and_formalia() {
        ...
        check = read_varilen_natural<Z>(rdptr);
        assert(check == 47);
        check = read_varilen_integer<Z>(rdptr);
        assert(check == -47);
        check = read_varilen_natural<Z>(rdptr);
        assert(check == 71);
        check = read_varilen_integer<Z64>(rdptr);
    ...
    void write_page_header_and_formalia() {
        ...
        write_varilen_integer<Z64>(wrpos, THE_BIG_NEG);
...
##     ##  ###  ###           ######     ##     ######          ####  #######
    */
    ...
    arbitrary_return_code persist_to_storage() {
        ...
        N bytes_to_write = raw_buffer.buffered_count(); // non_filed_ptr;    //
        // front_of(raw_buffer); //
        // raw_buffer.begin();
    ...
    L immediate_mode_ = false;
    L write_mode_ = false;
    N accepted_missing_files_count_ = 0;

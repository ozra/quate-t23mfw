#ifdef PRICEHUNTER_MAIN_INSTANCE
...
template <class T>
int run_specific(string broker_id, string symbol, QuantTime start_date, QuantTime end_date,
    ...
    ActLikeTrueEvery time_to_report(1);

    int t_delta = 0;
    int t_delta_thresh = 55; // 1 * 1000; //10 * 1000;
    int tick_count = 0;
    int above_thresh_tick_count = 0;
    int avg_t_delta = 0;
    int max_t_delta = 0;

    QSDF_Ticks_T23M dest_seq_write{
        true
...
int seqdacon_run(string broker_id, string symbol, pxt::ptime start_date,
               pxt::ptime end_date, bool verify_data_only, bool verbose) {
    if (broker_id == "DUKASCOPY") {
        return run_specific<QSDF_Ticks_Dukascopy>(
...
int main(int argc, char** argv)
{
    ...
    cerr << "Init Quant Price Hunter for quant sequential data conversions " <<
         argc
         << " " << argv << "\n";

...
class QuantBufferSynchronizedHeap {
   public:
    QuantBufferSynchronizedHeap(size_t default_buf_size);
    ~QuantBufferSynchronizedHeap();

    // auto add( AbstractQuantBuffer &buf ) -> QuantBufferSynchronizedHeap &;
    // auto add( AbstractQuantBuffer &buf, size_t size_capacity ) ->
    // QuantBufferSynchronizedHeap &;
...
QuantBufferSynchronizedHeap::QuantBufferSynchronizedHeap(size_t default_buf_size)
...
QuantBufferSynchronizedHeap::~QuantBufferSynchronizedHeap() {
    ...
    cerr << "/ QuantBufferSynchronizedHeap::~QuantBufferSynchronizedHeap - - "
            "DESTRUCTOR - -"
         << "\n\n";
}
...
// auto QuantBufferSynchronizedHeap::add( AbstractQuantBuffer &buf, int
// size_capacity )
//    -> QuantBufferSynchronizedHeap&
void QuantBufferSynchronizedHeap::add(QuantBufferAbstract& buf,
...
arbitrary_numeric_code QuantBufferSynchronizedHeap::allocate() {
    ...
    cerr << "QuantBufferSynchronizedHeap::allocate() total size to alloc: "
         << total_byte_size << "\n";
...
void QuantBufferSynchronizedHeap::advance() {
    ...
    for (auto buf : buffer_jar.buffers) {
        ...
        // cerr << "QuantBufferSynchronizedHeap::advance - ptrs BEFORE are: " <<
...
// buf->frontier_ptr += sizeof(QuantTypeSized);
    ...
    if (LIKELY(buffer_to_move == nullptr)) {
        ...
        cerr << "\nQuantBufferSynchronizedHeap::advance() - It's time to move "
                "a buf around!!!"
             << "\n";

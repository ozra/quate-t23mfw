...
class QSDF_Ticks_Dukascopy
    : public QSDF_Ticks_Abstract
{
  public:
    QSDF_Ticks_Dukascopy()
        : raw_buffer_((ff_size_t)5000) //,
          // decode_buffer ( (ff_size_t) 10000 )
    {};
  ...
  private:
    ...
    const natural session_break_duration_in_chunk_units = 24 * 2;
    std::string symbol;
...
bool QSDF_Ticks_Dukascopy::init(std::string p_broker_id,
                                std::string p_symbol)
{
    symbol = p_symbol;
    point_decimals =
...
void QSDF_Ticks_Dukascopy::reset_time_pos()
{
    chunk_time_pos_ =
...
arbitrary_return_code QSDF_Ticks_Dukascopy::loadNextChunk()
{
    //#ifdef IS_DEEPBUG
    _Dn("loadNextChunk()");
    //#endif
...
bool QSDF_Ticks_Dukascopy::readTick(QuantTick & tick)
{
    // cerr << "readTick()" << "\n";
    //#ifdef IS_DEBUG
...
// cerr << "get tick reference" << "\n";
    ...
    // // + ms;
    tick.time =
        chunk_time_pos_ + pxt::millisec(readBigEndian32<uint32_t>(buf_ptr));
    buf_ptr += 4;
    tick.last_price = tick.ask =
                          readBigEndian32<uint32_t>(buf_ptr) * point_decimals;
    buf_ptr += 4;
    tick.bid = readBigEndian32<uint32_t>(buf_ptr) * point_decimals;
    buf_ptr += 4;
    tick.ask_volume = readBigEndian32<real32>(buf_ptr);
    buf_ptr += 4;
    tick.bid_volume = readBigEndian32<real32>(buf_ptr);
    buf_ptr += 4;
//      cerr << "Specific decode Tick is: " << tick.ask << ", " << tick.bid <<
// ", " << tick.ask_volume << " at " << 47 << "/" << byte_buffer_size << "\n";
// cerr << "QSDF_Ticks_Dukascopy::readTick: " << tick.time << "

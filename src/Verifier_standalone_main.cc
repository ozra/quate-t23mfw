...
Z run_specific(S broker_id, string symbol, QuantTime start_date, QuantTime end_date,
               bool verify_data_only, bool verbose, N max_errors) {
    ...
    Z t_delta = 0;
    Z t_delta_thresh = 0; // 55; // 1 * 1000; //10 * 1000;
    Z tick_count = 0;
    ...
    while (true) {
        ...
        Z time_diff = qts::millis(tick_B.time) - qts::millis(tick_A.time);
        ...
        Z allowed_time_diff = 100;
        R allowed_price_diff = 0.00001;
        R allowed_volume_diff = 0.01;

        // correlation between A data and B data
...
Z verify_run(S broker_id, string symbol, pxt::ptime start_date, pxt::ptime end_date,
             bool verify_data_only, bool verbose, N max_errors) {
    if (broker_id == "DUKASCOPY") {
        return run_specific<SQDI_Ticks_Dukascopy>(broker_id, symbol, start_date,
...
Z main(Z argc, char** argv) {
    ...
    desc.add_options()("help", "Show help")(
        ...
        "Verify _only_")("limit", po::value<N>()->default_value(10),
    ...
    Z ret = verify_run(broker_id, symbol, start_date, end_date,
                       verify_data_only, verbose, opts["limit"].as<N>());

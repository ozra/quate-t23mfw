...
template <class T>
Z run_specific(S broker_id,
               S symbol,
               QuantTime start_date,
               QuantTime end_date,
               bool verify_data_only,
               bool verbose) {
    ...
    T source_seq;
    source_seq.setDateRange(aligned_start_ts, aligned_end_ts);
    ...
    if (verify_data_only == false) {

        QuantSequentialData_T23MFWTicks dest_seq_write{
            true
        }; // *TODO* Change to flags, it's clearer ( WRITE_MODE )
        dest_seq_write.setDateRange(aligned_start_ts, aligned_end_ts);
        dest_seq_write.init(broker_id, symbol);
        // dest_seq | dest_seq::FlagsEnum::HISTORY_MODE;
        dest_seq_write.setBatchMode(true);
    ...
    QuantSequentialData_T23MFWTicks dest_seq_read{
        false
    }; // *TODO* Change to flags, it's clearer ( WRITE_MODE )
    _Dn(1);
    dest_seq_read.setDateRange(aligned_start_ts, aligned_end_ts);
    _Dn(2);
    dest_seq_read.init(broker_id, symbol);
    _Dn(3);
    source_seq.reset_time_pos();
    _Dn(4);
    ...
    while (true) {
        _Dn(5);
        ret = source_seq.readTick(ref_tick);
        if (ret == false) {
            break;
        }
        if (ref_tick.time > aligned_end_ts) {
            break;
        }

        _Dn(6);
        ret = dest_seq_read.readTick(tick);
        if (ret == false) {
            break;
        }
        if (tick.time > aligned_end_ts) {
        ...
        if (tick.time != ref_tick.time || tick.ask != ref_tick.ask ||
            tick.bid != ref_tick.bid ||
            tick.ask_volume != ref_tick.ask_volume ||
            tick.bid_volume != ref_tick.bid_volume) {
            cerr << "DISCREPANCY: \n" << ref_tick.time << ": "
                 << "ref:" << ref_tick.to_str() << "\n" << tick.time << ": "
                 << "vs:" << tick.to_str() << "\n";
        }
...
Z seqdacon_run(S broker_id,
    ...
    if (broker_id == "DUKASCOPY") {
        return run_specific<QuantSequentialData_DukascopyTicks>(
...
Z main(Z argc, char** argv) {
    ...
    desc.add_options()("help", "Show help")(
        ...
        "broker",
        po::value<std::string>()->default_value("DUKASCOPY"),
        "Which brokers data to work upon")(
        "symbol",
        po::value<std::string>()->default_value("EURUSD"),
        "Which Symbol to convert")(
        "start",
        po::value<std::string>()->default_value("2013-12-01"),
        "Start timepoZ ( YYYY-MM-DD[ HH:MM[:SS]] )")(
        "end",
        po::value<std::string>()->default_value("2014-01-01"),
    ...
    S start_date_str = opts["start"].as<std::string>();
    ...
    S end_date_str = opts["end"].as<std::string>();

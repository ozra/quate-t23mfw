  ...
  public:
    ...
    inline void handleAndCommitValue(QuantBuffer<QuantReal>& in_buf)
    {
        ...
        if (in_buf.size <= usable_length) {
            ...
            if (usable_length <
                2) {    // +1 for CLOSED bar, +1 for lowest_so_far fallen out compare
                result |= 0.0;
                // *TODO*
        ...
        if (in_buf < lowest_so_far) {
            ...
            lowest_so_far = in_buf;
                 ...
                 lowest_so_far) {       // If value falling out of zone is lower, we're good with what we have
